...
2 * pi *6378   #pi value is  constants
...
fruits <- c("Banana","Apple","Orange","Strawberry") #vector in character
class(fruits)  
        
n1 <- c(1,2,3,4)    #vector of numerical
n2<- c(T,F,TRUE, FALSE)  
class(n1)
class(n2)

mix <- c(81,5.2,TRUE,"Mango",5L, F) # Heterogeneous Vectors
class(mix)  #class type in not found in mix
...
seq(1,50,by=3)  # bymeans increment
...
rep(45,7)  # Repetition
rep("Mango",5)  # first number or variable , and second is how many time


sample(1:50,8) # Random Sample
...
x<-c(2,0,0,4) # Indexing / Accessing Vector elements
x[1]
x[c(1)]
...
marks<-c(60,70,80,50,90) # Relational operators with vectors
